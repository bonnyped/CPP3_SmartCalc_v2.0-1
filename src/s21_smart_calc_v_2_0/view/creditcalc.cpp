...
void CreditCalc::on_CalcCredit_clicked() {
  ...
  if (ui->radioButtonAnnuitete->isChecked()) {
    ...
    ui->lineEditMonthlyPaymentResult->setText(
        QString::number(annuitet_calc.getOutput().max_monthly_pay_, 'f', 2));
    ui->lineEditOverpayResult->setText(
        QString::number(annuitet_calc.getOutput().overpayment_, 'f', 2));
    ui->lineEditTotalPaymentResult->setText(
        QString::number(annuitet_calc.getOutput().total_payment_, 'f', 2));
  } else {
    s21::ControllerVariedCalc varied_calc({loan_body, term, interest});
    varied_calc.calcResultInModel();
    QString max_min_pay =
        QString::number(varied_calc.getOutput().max_monthly_pay_, 'f', 2);
    max_min_pay += " ... ";
    max_min_pay +=
        QString::number(varied_calc.getOutput().min_monthly_pay_, 'f', 2);
    ui->lineEditMonthlyPaymentResult->setText(max_min_pay);
    ui->lineEditOverpayResult->setText(
        QString::number(varied_calc.getOutput().overpayment_, 'f', 2));
    ui->lineEditTotalPaymentResult->setText(
        QString::number(varied_calc.getOutput().total_payment_, 'f', 2));
  }
}
